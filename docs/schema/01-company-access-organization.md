...
**確認狀態：已確認採用；欄位待後續逐欄確認。**

**資料表用途：** 保存可登入系統的全域帳號與驗證資料。帳號資料不得直接併入 `employees`。

**目前已確認的責任邊界：** 同一登入帳號可以獨立於特定公司與員工存在；密碼只保存單向雜湊，不保存明文，也不設計為可解密內容。

**設計理由：** 登入身分與員工人事主檔生命週期不同。使用者可能加入多家公司，也可能是外部會計或管理者而不是員工；員工離職亦不應抹除帳號及歷史操作紀錄。

> 本輪只確認表名、用途及責任邊界；先前提出的 `username`、`password_hash`、狀態與時間欄位仍屬待確認內容，不視為定案欄位。

### `company_users`

**確認狀態：已確認採用；欄位待後續逐欄確認。**

**資料表用途：** 表示登入帳號加入某一公司的成員關係，作為全域 `users` 與公司 Tenant 之間的邊界；必要時再連結該公司的員工資料。

**目前已確認的責任邊界：** 公司成員關係與全域登入帳號分離，使一個帳號可加入多家公司，且非員工身分的外部協作者也能成為公司成員。

**設計理由：** 若把 `company_id` 或員工資料直接放入 `users`，會限制一個帳號只能屬於一家公司，也會把登入、安全與人事資料耦合。

> 本輪只確認表名、用途及責任邊界；`company_id`、`user_id`、`employee_id` 等曾討論欄位尚待逐欄確認，不在此提前定案。

### 尚未連帶確認

- `company_user_roles` 尚未定案；不能因 `users` 與 `company_users` 已確認而自動視為已確認。
- 新增員工時是否建立帳號、欄位必填性、帳號啟用方式及角色授予流程仍待後續 UI 與 Schema 一起確認。
